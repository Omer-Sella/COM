...
if ~isempty(varargin) % process case where file names are passed in function call
    if strfind(upper(char(varargin(1))),'EXTERNAL_IR') ~= 0
        ...
        if length(varargin) < xtk +1 % check that number of varargin arguments passed is at least number of crosstalk files+1 (thru)
            sprintf('number of cross talk files is ')
            sprintf('%d',xtk)
            sprintf('number of varargin is ')
            sprintf('%d',length(varargin))
...
while (OP.RX_CALIBRATION==1 || DO_ONCE==true)
    ...
    for package_testcase_i = 1:length(OP.pkg_len_select)
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [PDF,CDF,Noise_Struct]=Create_Noise_PDF(A_s,param,fom_result,chdata,OP,sigma_bn);
        ...
        if 1
            fprintf(' IL(VIP to VMP) aka die to die loss = %g dB \n',output_args.VIP_to_VMP_IL_dB_at_Fnq)
            fprintf('run time = %g min \n',output_args.rtmin)
        end
...
function [PDF,CDF,NS]=Create_Noise_PDF(A_s,param,fom_result,chdata,OP,sigma_bn)
...
for i=1:param.number_of_s4p_files
    ...
    if isequal(chdata(i).type, 'THRU')
        ...
        % the code above yields incorrect results if parameter f2 (which is unrelated to loss) is not set correctly.
...
%         total_loss=-20*log10(abs(SDDp2p(i).Parameters(2,1,nq_indx)));
        %total_loss=-20*log10(abs(chdata(i).sdd21(nq_indx))); % RIM 06-15-2022 total loss is now uneq VTF (eq 93a-19)
        total_loss = interp1(chdata(i).faxis, -20*log10(abs(chdata(i).sdd21)), 1/param.ui/2);
        Nq_loss=chdata(i).ILatNq;
        output_args.VIP_to_VMP_IL_dB_at_Fnq=total_loss;
...
%                  plot(faxis_GHz, 20*log10(abs(squeeze(chdata(i).sdd21))), 'r--', 'Disp','VTF')
                ylim(get(gca, 'ylim'));
                %                             plot(faxis_GHz(index_f1:index_f2),chdata(i).fit(index_f1:index_f2),'g','Disp','IL fit')
                plot(faxis_GHz, chdata(i).fit_orig,'g','Disp','IL fit') % Adee Ran 2022-08-28
...
function [ Vfiltered, Cmod ] = force( V ,param, OP , ix, C,return_V)
...
%   and most combinations of "Cmod" result in illegal combinations that do not need
%   Vfiltered to be calculated
% test with load('SBR_FIR_resp.mydata','-mat')
%% set up parameters ------------------------------------------------------
if ~exist('cursor_gain','var'), cursor_gain=0; end
if ~exist('return_V','var'), return_V=1; end
...
function [s21p,SCH,sigma_ACCM_at_tp0] = s21_pkg(chdata, param, OP, channel_number,mode)
