...
while (OP.RX_CALIBRATION==1 || DO_ONCE==true)
    ...
    for package_testcase_i = 1:length(OP.pkg_len_select)
        ...
        if OP.dynamic_txffe
            fom_result = optimize_fom_Dynamic_txffe(OP,param, chdata, sigma_bn,do_C2M);
        else
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [PDF,CDF,Noise_Struct]=Create_Noise_PDF(A_s,param,fom_result,chdata,OP);
...
for i=1:param.number_of_s4p_files
    ...
    uneq_ir=chdata(i).uneq_imp_response;
...
function [PDF,CDF,NS]=Create_Noise_PDF(A_s,param,fom_result,chdata,OP)
...
output_args.VIP_to_VIM_IL_dB_at_Fnq=[];
...
for i=1:param.number_of_s4p_files
    ...
    if isequal(chdata(i).type, 'THRU')
        ...
        %nq_indx=find(round(chdata(i).faxis,4,'significant') >= round(1/param.ui/2,4,'significant'),1, 'first');
        %finding nq_indx without using "roufnd":
        [tmp,nq_indx]=min(abs(chdata(i).faxis-1/param.ui/2));
        nq_indx=min(nq_indx,length(chdata(i).fit_orig));% freq array may not contain Fb/2 exactly RIM 06-09-21
        chdata(i).fit_ILatNq=-chdata(i).fit_orig(nq_indx);
        chdata(i).ILatNq=-20*log10(abs(chdata(i).sdd21f(nq_indx)));
        fit_loss=chdata(i).fit_ILatNq;
        output_args.fitted_IL_dB_at_Fnq = fit_loss;
        if OP.include_pcb
            cable_loss=-20*log10(abs(chdata(i).sdd21_orig(nq_indx)));
            loss_with_PCB=-20*log10(abs(chdata(i).sdd21_raw(nq_indx)));
            output_args.cable__assembley_loss=cable_loss;
            output_args.loss_with_PCB=loss_with_PCB;
        end
        total_loss=-20*log10(abs(SDDp2p(i).Parameters(2,1,nq_indx)));
        Nq_loss=chdata(i).ILatNq;
        output_args.VIP_to_VIM_IL_dB_at_Fnq=total_loss;
        ...
        if OP.COMPUTE_RILN
            ...
            %                         [TD_RILN]=get_RILN_cmp_td(chdata(1).sdd21_orig,RIL_struct,chdata(i).faxis,OP,param,1);% not vetted
            %                         FOM_TDRILN= TD_RILN.SNR_ISI_FOM_PDF; % Hansel 10/18/2021
        ...
        if OP.DEBUG
            if OP.DISPLAY_WINDOW
                ...
                subplot(3,1,1)
                title('Losses')
                plot(faxis_GHz, 20*log10(abs(squeeze(chdata(i).sdd21f))), 'b', 'LineWidth', 3, 'Disp','IL w/o pkg or board')
                hold on
                plot(faxis_GHz, 20*log10(abs(squeeze(SDDp2p(i).Parameters(2,1,:)))), 'b--', 'Disp','IL with pkgs')
                ylim(get(gca, 'ylim'));
                %                             plot(faxis_GHz(index_f1:index_f2),chdata(i).fit(index_f1:index_f2),'g','Disp','IL fit')
                plot(faxis_GHz(index_f1:index_f2),chdata(i).fit_orig,'g','Disp','IL fit')
...
if OP.DEBUG && OP.DISPLAY_WINDOW
    ...
    if param.number_of_s4p_files > 1
        %                     scale=scale=A_fext/A_thru;% added for crosstalk scaling
...
function H_tw=Tukey_Window(f,param)
fb=param.fb;
fr=param.f_r*param.fb;
fperiod=2*(fb-fr);
...
switch OP.include_pcb
    ...
    case 2
        [ s11out, s12out, s21out, s22out ]=combines4p( chdata.sdd11_raw, chdata.sdd21_raw, chdata.sdd12_raw, chdata.sdd22_raw, s11rx, s12rx, s21rx, s22rx);
end
...
function [idx]=findbankloc(hisi,idx_st,idx_en,tap_bk,curval,bmaxg)
...
function [idx]=findbankloc_DEVELOPMENT(hisi,idx_st,idx_en,tap_bk,curval,bmaxg,N_bg)
% [idx]=findbankloc(hisi,idx_st,idx_en,tap_bk)
% find the location of the DFE bank
% hisi: waveform with cursor values;
% idx_st: starting index;
...
% Adam Gregroy modifications of floatingDFE, Jingbo Li 5/16/2019
% COOP = 1 co-optimize banks , -0 sequenatial optmization ; remove

% idx_st=20;    % N_b number of fixed dfe taps
% nb_bk=4;      % N_bg number of floating tap groups. 1 2 or 3 right now
% tap_bk=6;     % N_bf number of floating taps per group
% curval=1;     % hsis(cursor_i)
% bmaxg=0.3;    % bmaxg limit for the floating taps
% dfe_delta=0;  % param.dfe_delta need to pass
% idx_en=90;    % N_bmax number of ui for the max reach of the floating taps

if nargin<8, dfe_delta=0;end

%new code making the entire vector of dfe taps'
tap_loc=zeros(1,N_bg*N_bf);
bank_loc=zeros(1,N_bg);
tap_coef=zeros(1,length(hisi));
b=zeros(1,length(hisi));
%hisi_ref is used for finding dfe taps.  whenever a bank is applied, those
%locations in hisi_ref are set to exactly 0.  This is to fix an issue where
%small bmaxg can create a situation where the same location may be used
%more than once even after DFE is applied to hisi.
hisi_ref=hisi;
for ii=1:N_bg
    % [ahealey] The start index for the floating tap search should be the
    % tap *following* the last fixed tap.
    bank_loc(ii)=findbankloc(hisi_ref,N_b+1,N_bmax,N_bf,curval,bmaxg);
    % bank_loc(ii)=findbankloc(hisi_ref,N_b,N_bmax,N_bf,curval,bmaxg);
    % [ahealey] End of modification.
    %don't need tap_loc anymore, but still populating it
    tap_loc((ii-1)*N_bf+1:ii*N_bf)=bank_loc(ii):bank_loc(ii)+N_bf-1;

    [hisi,coef,hisi_ref]=applyDFEbk(hisi,hisi_ref,bank_loc(ii),N_bf,curval,bmaxg,dfe_delta);
    tap_coef(bank_loc(ii):bank_loc(ii)+N_bf-1)=coef;
end
...
function [tap_loc,tap_coef,hisi,b]=floatingDFE_DEVELOPMENT( hisi, N_b, N_bf, N_bg, N_bmax, bmaxg, curval, dfe_delta)
...
[tap_loc]=findbankloc_DEVELOPMENT(hisi,N_b+1,N_bmax,N_bf,curval,bmaxg,N_bg);
...
Sout=H_i;
function[ s11out, s12out, s21out, s22out ] = makeTXpkg(faxis,param)
% create tx package for PTDR.
...
L_comp = [0, 0];
C_bump = [0, 0];
if isfield(param, 'L_comp')
    L_comp = param.L_comp;
end
if isfield(param, 'C_bump')
    C_bump = param.C_bump;
end
if isscalar(L_comp), L_comp = L_comp*[1, 1]; end
if isscalar(C_bump), C_bump = C_bump*[1, 1]; end
% [ahealey] End of modifications.
[~, mele]=size(param.z_p_next_cases);
switch mele
    case 1
        Zc1(1)=param.pkg_Z_c(1);% tx impedance
        Zc1(2)=param.pkg_Z_c(2);% rx
        Zp1(1)=param.Pkg_len_TX;% tx mm (length)
        Zp1(2)=param.Pkg_len_RX;% rx
        % [ahealey] Add series L shunt C to the on-die termination model.
        % [ s11out, s12out, s21out, s22out ]= make_pkg(faxis, Zp1(1), C_diepad(1), C_pkg_board(1),Zc1(1), param);
        [ s11out, s12out, s21out, s22out ]= make_pkg(faxis, Zp1(1), C_diepad(1), C_pkg_board(1),Zc1(1), param, L_comp(1), C_bump(1));
        % [ahealey] End of modification.
    case 4
        Zc1=[ param.pkg_Z_c(1,1) param.pkg_Z_c(2,1)] ;% Tx Rx impedance
        Zp1=[ param.Pkg_len_TX(1) param.Pkg_len_RX(1)] ;% Tx Rx length
        Zc2 =[ param.pkg_Z_c(1,2) param.pkg_Z_c(2,2)] ;
        Zp2 = [ param.Pkg_len_TX(2) param.Pkg_len_RX(2) ] ;
        Zc3 =[ param.pkg_Z_c(1,3) param.pkg_Z_c(2,3)] ;
        Zp3 = [ param.Pkg_len_TX(3) param.Pkg_len_RX(3) ] ;
        Zc4 = [ param.pkg_Z_c(1,4) param.pkg_Z_c(2,4)] ;
        Zp4 = [ param.Pkg_len_TX(4) param.Pkg_len_RX(4) ] ;
        Cv = [ param.C_v(1) param.C_v(2)  ];
        %
        % [ahealey] Add series L shunt C to the on-die termination model.
        % [ s11outa, s12outa, s21outa, s22outa ]= make_pkg(faxis, Zp1(1),  C_diepad(1),0 ,Zc1(1), param);
        [ s11outa, s12outa, s21outa, s22outa ]= make_pkg(faxis, Zp1(1),  C_diepad(1),0 ,Zc1(1), param, L_comp(1), C_bump(1));
        % [ahealey] End of modification.
        %
        [ s11outb, s12outb, s21outb, s22outb ]= make_pkg(faxis, Zp2(1) , 0, 0 ,Zc2(1), param); % add pth
        [ s11outc, s12outc, s21outc, s22outc ]  = combines4p(  s11outa, s12outa, s21outa, s22outa, s11outb, s12outb, s21outb, s22outb    );
        %
        %
        [ s11outd, s12outd, s21outd, s22outd ]= make_pkg(faxis, Zp3(1) , 0, Cv(1) ,Zc3(1), param); % add pth
        [ s11oute, s12oute, s21oute, s22oute ]  = combines4p(  s11outc, s12outc, s21outc, s22outc, s11outd, s12outd, s21outd, s22outd    );
        %
        [ s11outf, s12outf, s21outf, s22outf ]= make_pkg(faxis, Zp4(1) , 0, C_pkg_board(1), Zc4(1), param); % add pth
        [ s11out, s12out, s21out, s22out ]  = combines4p(  s11oute, s12oute, s21oute, s22oute, s11outf, s12outf, s21outf, s22outf    );
    otherwise
        error('package symtax error')
end


%% functions for estimating HEO or EW
...
cm4_values = param.tx_ffe_cm4_values;
cm3_values = param.tx_ffe_cm3_values;
cm2_values = param.tx_ffe_cm2_values;
cm1_values = param.tx_ffe_cm1_values;
cp1_values = param.tx_ffe_cp1_values;
cp2_values = param.tx_ffe_cp2_values;
cp3_values = param.tx_ffe_cp3_values;
num_txffe_runs=length(cm4_values)* length(cm3_values)*length(cm2_values)*length(cm1_values)*length(cp1_values)*length(cp2_values)*length(cp3_values);
    ...
    for Gffe_index=1:length(Gffe_values)
        ...
        for ctle_index=1:length(gdc_values)
            ...
            for  g_LP_index=1:lf_indx
                ...
                for k_cm4=1:length(cm4_values)
                    cm4=cm4_values(k_cm4);
                    for k_cm3=1:length(cm3_values)
                        cm3=cm3_values(k_cm3);
                        for k_cm2=1:length(cm2_values)
                            cm2=cm2_values(k_cm2);
                            for k_cm1=1:length(cm1_values)
                                cm1=cm1_values(k_cm1);
                                for k_cp1=1:length(cp1_values)
                                    cp1=cp1_values(k_cp1);
                                    for k_cp2=1:length(cp2_values)
                                        cp2=cp2_values(k_cp2);
                                        for k_cp3=1:length(cp3_values)
                                            cp3=cp3_values(k_cp3);
                                            pxi=pxi+1;
                                            progress = pxi/runs;
                                            if OP.DISPLAY_WINDOW
                                            ...
                                            cur=5;
                                            if length(cm4_values) == 1  && cm4_values == 0; cm4c =[];cur=cur-1; else cm4c=cm4; end
                                            if length(cm3_values) == 1  && cm3_values == 0; cm3c =[];cur=cur-1;else cm3c=cm3;end
                                            if length(cm2_values) == 1  && cm2_values == 0; cm2c =[];cur=cur-1;else cm2c=cm2;end
                                            if length(cm1_values) == 1  && cm1_values == 0; cm1c =[];cur=cur-1;else cm1c=cm1;end
                                            if length(cp1_values) == 1  && cp1_values == 0; cp1c =[];else cp1c=cp1;end
                                            if length(cp2_values) == 1  && cp2_values == 0; cp2c =[];else cp2c=cp2;end
                                            if length(cp3_values) == 1  && cp3_values == 0; cp3c =[];else cp3c=cp3;end
                                            %only calculate cursor here
                                            %move full txffe declaration to after LOCAL_SEACH check for speedup
                                            txffe_cur=(1-abs(cm3)-abs(cm4)-abs(cm2)-abs(cm1)-abs(cp1)-abs(cp2)-abs(cp3));
                                            ...
                                            txffe = [cm4c, cm3c, cm2c, cm1c, txffe_cur  , cp1c, cp2c, cp3c];
                                            ...
                                            if OP.RxFFE
                                                ...
                                                if C(Ccur_i) < param.ffe_main_cursor_min, continue; end
                                                if param.ffe_post_tap_len ~=0
                                                    if abs(C(Ccur_i +1)) > param.ffe_post_tap1_max, continue; end
                                                    if (param.ffe_post_tap_len > 1)
                                                        if sum(abs(C((Ccur_i +2):end))  > param.ffe_tapn_max) , continue; end
                                                    end
                                                end
                                                if param.ffe_pre_tap_len ~=0
                                                    if abs(C(Ccur_i -1)) > param.ffe_pre_tap1_max, continue; end
                                                    if (param.ffe_pre_tap_len > 1)
                                                        %                                             if sum(abs(C((Ccur_i +2):end)) > param.ffe_tapn_max) , continue; end
                                                        if sum(abs(C(1:(Ccur_i - 2))) > param.ffe_tapn_max) , continue; end % 11.22.2018 Yasou Hadaka
                                                    end
                                                end
                                            ...
                                            if param.N_bg ~=0
                                                ...
                                                if OP.FloatingDFE_Development
                                                    [floating_tap_locations, floating_tap_coef, hisi, bmax]= floatingDFE_DEVELOPMENT( postcurors ,param.ndfe_passed,param.N_bf,param.N_bg,param.N_bmax, param.bmaxg, sbr(cursor_i), param.dfe_delta  );
                                                else
...
if OP.DEBUG ~=0
    if OP.DISPLAY_WINDOW && ~OP.RX_CALIBRATION
        ...
        plot(t,best_sbr/Symbol_Adj,'disp', 'Half Symbol Equalized end-to-end PR');
        hold on
        ...
        plot(t-param.ui-t(6),PRplt/Symbol_Adj,'r','disp', 'Half Symbol Unequalized end-to-end PR');
        stem(t(best_cursor_i),best_sbr(best_cursor_i)/Symbol_Adj,'g','disp','Cursor (sample point)');
        title(sprintf('Case %d', param.package_testcase_i));
        ylabel('volts')
        xlabel('seconds')

        plot(sampled_best_sbr_precursors_t-param.ui/param.samples_per_ui, sampled_best_sbr_precursors'/Symbol_Adj, 'kx', 'disp','Pre cursors');
        plot(sampled_best_sbr_postcursors_t-param.ui/param.samples_per_ui, sampled_best_sbr_postcursors'/Symbol_Adj, 'ko', 'disp','Post cursors');
        if param.ndfe_passed ~=0
            stem(sampled_best_sbr_dfecursors_t-param.ui/param.samples_per_ui,DFE_taps_mV(1:param.ndfe_passed)/Symbol_Adj','m', 'LineWidth',2,'disp','DFE-canceled cursors');
        end
        if param.N_bg ~=0
            stem(sampled_best_sbr_fdfecursors_t-param.ui/param.samples_per_ui,FDFE_taps_mV/Symbol_Adj, 'MarkerFaceColor','red','MarkerEdgeColor','m','LineWidth',1,'disp','FDFE-canceled cursors');
        end
        grid on
        legend show
        zoom xon
...
result.t=t;
result.sbr=best_sbr;
if OP.RxFFE, result.RxFFE=best_RxFFE;end
...
function result=optimize_fom_Dynamic_txffe(OP, param, chdata, sigma_bn,do_C2M)
%% input
% chdata(1).uneq_imp_response is the impulse response input expected to be normalized to At, peak drive voltage
% baud_rate - baud rate in seconds
% param.samples_per_ui = samples per UI of IR
    ...
    if param.package_testcase_i==1  && i==1
        if OP.TDR && i==1
            ...
            if OP.TDR_W_TXPKG
                ...
                %[ s11in, s12in, s21in, s22in ] = makeTXpkg(S.Frequencies,param);
                [ S.Parameters(1,1,:), S.Parameters(1,2,:), S.Parameters(2,1,:), S.Parameters(2,2,:)]  = ...
...
if length(Spar.Z0)>1 || ~isequal(Spar.Z0,50)
...
param.ffe_main_cursor_min=xls_parameter(parameter, 'ffe_main_cursor_min', true,0.7); % Rx FFE main cursor miminum
...
param.ffe_backoff=xls_parameter(parameter, 'ffe_backoff', true,4); % see if better zero foreced solution is better by backing off the number specified FFE taps one at at time
...
param.f_r = xls_parameter(parameter, 'f_r', true, 4); % feference receive filter in COM and in ICN/FOM_ILD calcs in terms of fb
...
OP.inc_reflect_board=xls_parameter(parameter, 'inc_reflect_board', false,0); % Not used. Default is zero.
...
OP.SAVE_CONFIG2MAT = xls_parameter(parameter, 'SAVE_CONFIG2MAT', false, 1); % If set to 1 (default) saves parameters in mat file. Requires DIAGNOSTICS to be set.
...
% idxLine = 1;
% while true
%     this_line=strtrim(data_file{idxLine});
%     if regexp(this_line, '^#', 'once')
%         % tests the option fileDataLine cell array to see if it contains...
%         if ~isempty(regexpi(this_line, '\sGHz')); freq_scale = 1e9; end
%         if ~isempty(regexpi(this_line, '\sMHz')); freq_scale = 1e6; end
%         if ~isempty(regexpi(this_line, '\sKHz')); freq_scale = 1e3; end
%         if ~isempty(regexpi(this_line, '\sHz'));  freq_scale = 1e0; end
%         if ~isempty(regexpi(this_line, 'RI')) && ~isempty(regexpi(this_line, 'R'))
%             format = 'complex';
%         end
%         if ~isempty(regexpi(this_line, 'MA')) && ~isempty(regexpi(this_line, 'R'))
%             format = 'linpolar';
%         end
%         if ~isempty(regexpi(this_line, 'DB')) && ~isempty(regexpi(this_line, 'R'))
%             format = 'dBpolar';
%         end
%
...
%                 'FROM THE OPTIONS LINE IN THE S2P FILE']);
...
% end
%
% col_1=1; col_2=2; % This is to maintain current indexing compatibility as well as readability
% schFreqAxis = zeros(1,int8((length(data_file)-idxLine)/2)); % preallocated array length for memory and time purposes
% idxNetParams = 1; % each 2-port network parameter data point set will receive an index
%
% freqCounter = 0; % variable that will be used to capture number of freq points.
%
% while true
%     this_line=strtrim(data_file{idxLine});
%     if isempty(this_line) || ~isempty(regexp(this_line, '^!', 'once'))
%         idxLine = idxLine+1;
%     else
%         for row=1:2
%             %             dataPointsLine = eval (['[' data_file{idxLine} ']']);
%             this_line=strtrim(data_file{idxLine});
%             dataPointsLine= sscanf(this_line, '%f'); % Close V2015 memory leak - Adam Gregory
%             if mod(length(dataPointsLine),4)==1
...
%             end
%             if mod(length(dataPointsLine),4)~=0  % sanity check to make sure the network parameter data point line structure is correct
%                 msg = sprintf(['\n'...
%                     '\n\t ******************************************************' ...
%                     '\n\t ** There is an error in the s2p network parameter   **' ...
...
%                 pause;
%                 pause off;
%                 error( 'COM:reads_2p:SparamFileDataError', 'INCORRECT S4P NETWORK PARAMETER DATA POINT STRUCTURE');
%             else
%                 %map = [[ 1 2 3 4];[ 7 8 5 6]];
%                 %AJG:  fix mapping to give [S11 S12; S21 S22]
%                 map = [[1 2 5 6] ; [3 4 7 8]];
%                 switch format
%                     case 'complex'
%                         sch(idxNetParams, ports(row), ports(col_1)) = dataPointsLine(map(row,1)) + 1i*dataPointsLine(map(row,2)); %#ok<AGROW>
%                         sch(idxNetParams, ports(row), ports(col_2)) = dataPointsLine(map(row,3)) + 1i*dataPointsLine(map(row,4)); %#ok<AGROW>
%                     case 'linpolar'
%                         sch(idxNetParams, ports(row), ports(col_1)) = dataPointsLine(map(row,1)) * exp(1i*dataPointsLine(map(row,2))*(pi/180)); %#ok<AGROW>
%                         sch(idxNetParams, ports(row), ports(col_2)) = dataPointsLine(map(row,3)) * exp(1i*dataPointsLine(map(row,4))*(pi/180)); %#ok<AGROW>
%                     case 'dBpolar'
%                         sch(idxNetParams, ports(row), ports(col_1)) = 10^(dataPointsLine(map(row,1))/20) * exp(1i*dataPointsLine(map(row,2))*(pi/180)); %#ok<AGROW>
%                         sch(idxNetParams, ports(row), ports(col_2)) = 10^(dataPointsLine(map(row,3))/20) * exp(1i*dataPointsLine(map(row,4))*(pi/180)); %#ok<AGROW>
%                     otherwise
%                 end
%             end
%
...
function [data, SDD, SDC, SCC ] = read_p4_s4params(infile, plot_ini_s_params, plot_dif_s_params, ports,OP)
%% FUNCTION :: read_sp4_sparams
%
% Description
%   Read the fid of single-ended 4-port complex S-parameters
        ...
        switch lower(chdata(i).ext)
            ...
            case '.s4p'
                ...
                if param.package_testcase_i==1 % added to speed up cases e.g. don't read file in twice
                    [Sch, SDDch, SDCch] = read_p4_s4params(chdata(i).filename,  0, 0, param.snpPortsOrder, OP);
    ...
    if ~param.FLAG.S2P
        if  OP.INC_PACKAGE ~= 0 || (OP.RX_CALIBRATION == 1  && i==1)
            if (OP.RX_CALIBRATION == 1  && i==2)
                ...
                %new way with single function for both DD and DC
                [chdata(i).sdd21p,SDDp2p(i)]= s21_pkg_tester(chdata(i), param, OP, i);
                chdata(i).sdd21=chdata(i).sdd21p;
                if 1 % for AC CM noise inclusion
                    [chdata(i).sdc21p,SDCp2p(i),chdata(i).sigma_ACCM_at_tp0]= s21_pkg_tester(chdata(i), param, OP, i,'dc');
                    chdata(i).sdc21=chdata(i).sdc21p;
                end
                %old way
                %                 if param.kappa1 == 1 && param.kappa2 == 1
                %                     [chdata(i).sdd21p,SDDp2p(i)]= s21_pkg(chdata(i).sdd21_raw, chdata(i).sdd12_raw, chdata(i).sdd11_raw, chdata(i).sdd22_raw, chdata(i).faxis, param, OP, chdata(i).type, i, chdata(i));
                %                     chdata(i).sdd21=chdata(i).sdd21p;
                %                     if 1 % for AC CM noise inclusion
                %                        [chdata(i).sdc21p,SDCp2p(i),chdata(i).sigma_ACCM_at_tp0]= sdc21_pkg(chdata(i).sdc21_raw, chdata(i).sdc12_raw, chdata(i).sdc11_raw, chdata(i).sdc22_raw, chdata(i).faxis, param, OP, chdata(i).type, i, chdata(i));
                %                         chdata(i).sdc21=chdata(i).sdc21p;
                %                     end
                %                 else % remove return loss at port to determine reflectionless isi
                %                     chadata_RL_wo_TXRL=chdata(i).sdd11_raw;
                %                     chadata_RL_wo_RXRL=chdata(i).sdd22_raw;
                %                     if param.kappa1 == 0
                %                         chadata_RL_wo_TXRL= zeros(1, length(chdata(i).sdd22_raw));
                %                     else
                %                         chadata_RL_wo_TXRL=chdata(i).sdd11_raw*param.kappa1;
                %                     end
                %                     if param.kappa2 == 0
                %                         chadata_RL_wo_RXRL= zeros(1, length(chdata(i).sdd22_raw));
                %                     else
                %                         chadata_RL_wo_RXRL=chdata(i).sdd22_raw*param.kappa2;
                %                     end
                %                     [chdata(i).sdd21p,SDDp2p(i)]= s21_pkg(chdata(i).sdd21_raw, chdata(i).sdd12_raw, chadata_RL_wo_TXRL, chadata_RL_wo_RXRL, chdata(i).faxis, param, OP, chdata(i).type, i, chdata(i));
                %                     chdata(i).sdd21=chdata(i).sdd21p;
                %                     if 1 % for AC CM noise inclusion
                %                         [chdata(i).sdc21p,SDCp2p(i),chdata(i).sigma_ACCM_at_tp0]= sdc21_pkg(chdata(i).sdc21_raw, chdata(i).sdc12_raw, chdata(i).sdc11_raw, chdata(i).sdc22_raw, chdata(i).faxis, param, OP, chdata(i).type, i, chdata(i));
                %                         chdata(i).sdc21=chdata(i).sdc21p;
...
function [sdd21p,SCH] = s21_pkg(sdd21, sdd12, sdd11, sdd22, faxis, param, OP, channel_type, channel_number,chdatalocal)
    ...
    case 'NEXT'
        switch mele
            case 1
                Zc1(1)=param.pkg_Z_c(1);% tx impedance
                Zc1(2)=param.pkg_Z_c(2);% rx
                Zp1(1)=param.Pkg_len_NEXT;% tx mm (length)
                Zp1(2)=param.Pkg_len_RX;% rx
            case 4
                Zc1=[ param.pkg_Z_c(1,1) param.pkg_Z_c(2,1)] ;% Tx Rx impedance
                Zp1=[ param.Pkg_len_NEXT(1) param.Pkg_len_RX(1)] ;% Tx Rx length
                Zc2 =[ param.pkg_Z_c(1,2) param.pkg_Z_c(2,2)] ;
                Zp2 = [ param.Pkg_len_NEXT(2) param.Pkg_len_RX(2) ] ;
                Zc3 =[ param.pkg_Z_c(1,3) param.pkg_Z_c(2,3)] ;
                Zp3 = [ param.Pkg_len_NEXT(3) param.Pkg_len_RX(3) ] ;
                Zc4 = [ param.pkg_Z_c(1,4) param.pkg_Z_c(2,4)] ;
                Zp4 = [ param.Pkg_len_NEXT(4) param.Pkg_len_RX(4) ] ;
                Cv = [ param.C_v(1) param.C_v(2)  ];
            otherwise
                error('package symtax error')
        end
    case 'FEXT'
        switch mele
            case 1
                Zc1(1)=param.pkg_Z_c(1);% tx impedance
                Zc1(2)=param.pkg_Z_c(2);% rx
                Zp1(1)=param.Pkg_len_FEXT;% tx mm (length)
                Zp1(2)=param.Pkg_len_RX;% rx
            case 4
                Zc1=[ param.pkg_Z_c(1,1) param.pkg_Z_c(2,1)] ;% Tx Rx impedance
                Zp1=[ param.Pkg_len_FEXT(1) param.Pkg_len_RX(1)] ;% Tx Rx length
                Zc2 =[ param.pkg_Z_c(1,2) param.pkg_Z_c(2,2)] ;
                Zp2 = [ param.Pkg_len_FEXT(2) param.Pkg_len_RX(2) ] ;
                Zc3 =[ param.pkg_Z_c(1,3) param.pkg_Z_c(2,3)] ;
                Zp3 = [ param.Pkg_len_FEXT(3) param.Pkg_len_RX(3) ] ;
                Zc4 = [ param.pkg_Z_c(1,4) param.pkg_Z_c(2,4)] ;
                Zp4 = [ param.Pkg_len_FEXT(4) param.Pkg_len_RX(4) ] ;
                Cv = [ param.C_v(1) param.C_v(2)  ];
            otherwise
                error('package symtax error')
        end
...
switch mele
    case 1
        % [ahealey] Add series L shunt C to the on-die termination model.
        % [ s11in, s12in, s21in, s22in ]= make_pkg(faxis,  Zp1(2), C_diepad(2), C_pkg_board(2), Zc1(2), param);
        [ s11in, s12in, s21in, s22in ]= make_pkg(faxis,  Zp1(2), C_diepad(2), C_pkg_board(2), Zc1(2), param, L_comp(2), C_bump(2));
    case 4
        % [ s11ina, s12ina, s21ina, s22ina ]= make_pkg(faxis, Zp1(2), C_diepad(2),0 ,Zc1(2), param));
        [ s11ina, s12ina, s21ina, s22ina ]= make_pkg(faxis, Zp1(2), C_diepad(2),0 ,Zc1(2), param, L_comp(2), C_bump(2));
        % [ahealey] End of modifications.
        %
        [ s11inb, s12inb, s21inb, s22inb ]= make_pkg(faxis, Zp2(2) , 0, 0 ,Zc2(2), param); % add pth
        [ s11inc, s12inc, s21inc, s22inc ]  = combines4p(  s11ina, s12ina, s21ina, s22ina, s11inb, s12inb, s21inb, s22inb    );
        %
        [ s11ind, s12ind, s21ind, s22ind ]= make_pkg(faxis, Zp3(2) , 0, Cv(2) ,Zc3(2), param); % add pth
        [ s11ine, s12ine, s21ine, s22ine ]  = combines4p(  s11inc, s12inc, s21inc, s22inc, s11ind, s12ind, s21ind, s22ind    );
        %
        [ s11inf, s12inf, s21inf, s22inf ]= make_pkg(faxis, Zp4(2) , 0, C_pkg_board(2), Zc4(2), param); % add pth
        [ s11in, s12in, s21in, s22in ]  = combines4p(  s11ine, s12ine, s21ine, s22ine, s11inf, s12inf, s21inf, s22inf    );
end
...
if OP.IDEAL_TX_TERM || (OP.RX_CALIBRATION == 1 && channel_number == 2) || OP.include_pcb == 2
    ...
    gamma_tx=(R_diepad(param.Tx_rd_sel)-Z0)/(R_diepad(param.Tx_rd_sel)+Z0);% equation 93A-17
end
if OP.IDEAL_RX_TERM
    gamma_rx=0;
else
    gamma_rx=(R_diepad(param.Rx_rd_sel)-Z0)/(R_diepad(param.Rx_rd_sel)+Z0);% equation 93A-17
end
...
if OP.INC_PACKAGE==0
    ...
    if OP.RX_CALIBRATION == 1 && channel_number == 2
        ...
        [s11out_rx, s12out_rx, s21out_rx, s22out_rx ] = combines4p( sdd11, sdd21, sdd21, sdd22, s22out, s12out, s21out, s11out ); %#ok<ASGLU> % s22 is ball side of package
        SCH.Frequencies=faxis;
        ...
        if ~OP.IDEAL_TX_TERM
            [sdd11, sdd12, sdd21, sdd22] = combines4p( s11out, s12out, s21out, s22out, sdd11, sdd12, sdd21, sdd22 ); %#ok<ASGLU>
        end
        ...
        if ~OP.IDEAL_RX_TERM
            [sdd11, sdd12, sdd21, sdd22] = combines4p( sdd11, sdd12, sdd21, sdd22, s22in, s12in, s21in, s11in ); %#ok<ASGLU> % s22 is ball side of package
        else
            warning('do not use IDEAL_RX_TERM. Instead hard code package and TR parameters')
        end
        %% Equation 93A-18 and part of 93A-1: Ht fix in V290 identified by Bill Kirkland and Ed Frlan
        sdd21p= H_t.*sdd21.*(1-gamma_tx).*(1+gamma_rx)./(1.- sdd11.*gamma_tx - sdd22.*gamma_rx  -sdd21.^2.*gamma_tx.*gamma_rx +sdd11.*sdd22.*gamma_tx.*gamma_rx);
    end
    %     % for debug
    %     p(1 ,1, :)=sdd11;
    %     p(2 ,2, :)=sdd22;
...
function [s21p,SCH,sigma_ACCM_at_tp0] = s21_pkg_tester(chdata, param, OP, channel_number,mode)
% concatenates package reflections with s21,s11,and s22 with spec return loss (gammas)
% faxis is the frequency array
...
%Make TX Package
[ s11out, s12out, s21out, s22out]=make_full_pkg('TX',faxis,param,channel_type,mode);

%Make RX Package
%Important:  RX pkg doesn't change based on mode being dd or dc.  So no
%mode  argument is passed (keeps the default of dd)
[ s11in, s12in, s21in, s22in]=make_full_pkg('RX',faxis,param,channel_type);
...
if OP.INC_PACKAGE==0
    ...
    if OP.RX_CALIBRATION == 1 && channel_number == 2
        ...
        %% Equation 93A-18 and part of 93A-1: Ht fix in V290 identified by Bill Kirkland and Ed Frlan
        s21p= H_t.*s21.*(1-gamma_tx).*(1+gamma_rx)./(1.- s11.*gamma_tx - s22.*gamma_rx  -s21.^2.*gamma_tx.*gamma_rx +s11.*s22.*gamma_tx.*gamma_rx);
    end
...
%%
function [sdc21p,SCHDC,sigma_ACCM_at_tp0] = sdc21_pkg(sdc21, sdc12, sdc11, sdc22, faxis, param, OP, channel_type, channel_number,chdatalocal)
% concatenates package reflections with s21,s11,and s22 with spec return loss (gammas)
% faxis is the frequency array
% sdc21, sdc11, sdc22 are the corresponding array of differential parameters
% sdc121=ones(length(sdc21),1);
% sdc12=sdc21;
Z0=param.Z0;
sigma_ACCM_at_tp0=0;
...
switch mele
    case 1
        % [ahealey] Add series L shunt C to the on-die termination model.
        % [ s11out, s12out, s21out, s22out ]= make_pkg(faxis, Zp1(1), C_diepad(1), C_pkg_board(1),Zc1(1), param);*2);
        [ s11out, s12out, s21out, s22out ]= make_pkg(faxis, Zp1(1), C_diepad(1)*2, C_pkg_board(1),Zc1(1)*2, param, L_comp(1)/2, C_bump(1)*2 );
    case 4
        % [ s11outa, s12outa, s21outa, s22outa ]= make_pkg(faxis, Zp1(1),  C_diepad(1),0 ,Zc1(1), param);
        [ s11outa, s12outa, s21outa, s22outa ]= make_pkg(faxis, Zp1(1),  C_diepad(1)*2,0 ,Zc1(1)*2, param, L_comp(1)/2, C_bump(1)*2);
        % [ahealey] End of modifications.
        %
        [ s11outb, s12outb, s21outb, s22outb ]= make_pkg(faxis, Zp2(1) , 0, 0 ,Zc2(1)*2, param); % add pth
        [ s11outc, s12outc, s21outc, s22outc ]  = combines4p(  s11outa, s12outa, s21outa, s22outa, s11outb, s12outb, s21outb, s22outb    );
        %
        %
        [ s11outd, s12outd, s21outd, s22outd ]= make_pkg(faxis, Zp3(1) , 0, Cv(1)*2 ,Zc3(1)*2, param); % add pth
        [ s11oute, s12oute, s21oute, s22oute ]  = combines4p(  s11outc, s12outc, s21outc, s22outc, s11outd, s12outd, s21outd, s22outd    );
        %
        [ s11outf, s12outf, s21outf, s22outf ]= make_pkg(faxis, Zp4(1) , 0, C_pkg_board(1)*2, Zc4(1)*2, param); % add pth
        [ s11out, s12out, s21out, s22out ]  = combines4p(  s11oute, s12oute, s21oute, s22oute, s11outf, s12outf, s21outf, s22outf    );
end
param.Z0=param.Z0*2;% change tx reference back for DD mode
% RX package
switch mele
    case 1
        % [ahealey] Add series L shunt C to the on-die termination model.
...
if OP.IDEAL_TX_TERM || (OP.RX_CALIBRATION == 1 && channel_number == 2) || OP.include_pcb == 2
    ...
    gamma_tx=(R_diepad(param.Tx_rd_sel)/2-Z0/2)/(R_diepad(param.Tx_rd_sel)/2+Z0/2);% equation 93A-17 but for CM
end
if OP.IDEAL_RX_TERM
    gamma_rx=0;
else
    gamma_rx=(R_diepad(param.Rx_rd_sel)/2-Z0/2)/(R_diepad(param.Rx_rd_sel)/2+Z0/2);% equation 93A-17 but for CM
end
...
if OP.INC_PACKAGE==0
    ...
    if OP.RX_CALIBRATION == 1 && channel_number == 2
        %   for calibration do not include the transmitter package
        [s11out_rx, s12out_rx, s21out_rx, s22out_rx ] = combines4p( sdc11, sdc12, sdc21, sdc22, s22out, s12out, s21out, s11out ); %#ok<ASGLU> % s22 is ball side of package
        SCHDC.Frequencies=faxis;
        SCHDC.Parameters(1,1,:)=s11out_rx;
        SCHDC.Parameters(2,2,:)=s22out_rx;
        SCHDC.Parameters(1,2,:)=s12out_rx;
        SCHDC.Parameters(2,1,:)=s21out_rx;
        SCHDC.NumPorts=2;
        SCHDC.Impedance=100;
        %% Equation 93A-18
        sdc21p= s21out_rx.*(1-gamma_tx).*(1+gamma_rx)./(1.- s11out_rx.*gamma_tx - s22out_rx.*gamma_rx  -s21out_rx.^2.*gamma_tx.*gamma_rx +s11out_rx.*s22out_rx.*gamma_tx.*gamma_rx);
    else
        %% Equations 93A-4 to 93A-7
        if ~OP.IDEAL_TX_TERM
            [sdc11, sdc12, sdc21, sdc22] = combines4p( s11out, s12out, s21out, s22out, sdc11, sdc12, sdc21, sdc22 ); %#ok<ASGLU>
        end
        H_t=ones(1,length(faxis)); % .3bj compatibility
        if OP.IDEAL_TX_TERM ||  OP.T_r_filter_type == 1
            % for RITT testing with good termination as in some instruments
        ...
        %         H_t=ones(1,length(faxis)); % not sure if we need a H_t or not. Is the CM noise correlated to the edge rate?
        if ~OP.IDEAL_RX_TERM
            [sdc11, sdc12, sdc21, sdc22] = combines4p( sdc11, sdc12, sdc21, sdc22, s22in, s12in, s21in, s11in ); %#ok<ASGLU> % s22 is ball side of package
        else
            warning('do not use IDEAL_RX_TERM. Instead hard code package and TR parameters')
        end
        %% Equation 93A-18 and part of 93A-1: Ht fix in V290 identified by Bill Kirkland and Ed Frlan
        sdc21p= H_t.*sdc21.*(1-gamma_tx).*(1+gamma_rx)./(1.- sdc11.*gamma_tx - sdc22.*gamma_rx  -sdc21.^2.*gamma_tx.*gamma_rx +sdc11.*sdc22.*gamma_tx.*gamma_rx);
    end
    ...
    SCHDC.Frequencies=faxis;
    SCHDC.Parameters(1,1,:)=sdc11;
    SCHDC.Parameters(2,2,:)=sdc22;
    SCHDC.Parameters(1,2,:)=sdc12;
    SCHDC.Parameters(2,1,:)=sdc21;
    SCHDC.NumPorts=2;
    SCHDC.Impedance=25;
...
OP.SAVE_KEYWORD_FILE=0; % OP not passed ... maybe later
if OP.SAVE_KEYWORD_FILE
